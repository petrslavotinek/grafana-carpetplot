...
System.register(['d3', 'lodash', 'moment', 'jquery', '../fragments', './tooltip', '../formatting', '../x-axis-label-formats', '../theme-provider', '../color-modes', '../color-spaces', '../libs/d3-scale-chromatic/index'], function (_export, _context) {
    ...
    var axisBottom, d3AxisLeft, d3Color, scaleLinear, scaleSequential, scaleTime, d3Select, timeDay, timeFormat, timeHour, timeMonday, timeMonth, timeSaturday, _, range, moment, $, getFragment, CarpetplotTooltip, valueFormatter, labelFormats, themeProvider, colorModes, interpolationMap, d3ScaleChromatic, _slicedToArray, DEFAULT_X_TICK_SIZE_PX, X_AXIS_TICK_MIN_SIZE, Y_AXIS_TICK_PADDING, Y_AXIS_TICK_MIN_SIZE, MIN_SELECTION_WIDTH, LEGEND_HEIGHT, LEGEND_TOP_MARGIN, DO_NOT_ROUND, ROUND_DECIMALS;
    ...
    function link(scope, elem, attrs, ctrl) {
        ...
        function addCarpetplotSvg() {
            ...
            carpet = d3Select($carpet[0]).append('svg').attr('width', width).attr('height', height);
        }
        ...
        function addYAxis() {
            yScale = scaleTime().domain([moment().startOf('day').add(1, 'day'), moment().startOf('day')]).range([chartHeight, 0]);

            var yAxis = d3AxisLeft(yScale).ticks(getYAxisTicks()).tickFormat(function (value) {
        ...
        function getYAxisWidth() {
            ...
            return _.maxBy(axisText, function (text) {
        ...
        function addXAxis() {
            ...
            xScale = scaleTime().domain([xFrom, xTo]).range([0, chartWidth]);

            var xAxis = axisBottom(xScale).ticks(getXAxisTicks(xFrom, xTo)).tickFormat(timeFormat(panel.xAxis.labelFormat)).tickSize(chartHeight);
        ...
        function addDayTicks(posX, posY, range) {
            var ticks = axisBottom(xScale).ticks(range).tickSize(chartHeight);
        ...
        function addCanvas() {
            ...
            canvas = d3Select($carpet[0]).insert('canvas', ':first-child').attr('width', chartWidth).attr('height', chartHeight).style('left', yAxisWidth + 'px').style('top', margin.top + 'px');
        ...
        function addPoints() {
            ...
            var container = d3Select(customBase);
            ...
            var pointScale = scaleLinear().domain([fragment.count, 0]).range([chartHeight, 0]);
        ...
        function getColorScaleSpectrum(min, max) {
            ...
            return scaleSequential(colorInterpolator).domain(domain);
        }
        ...
        function getColorScaleCustom(min, max) {
            ...
            if (isDefined(firstBreakpoint)) {
                ...
                var fill = function fill(i, value) {
                    ...
                    range(i - last - 1).forEach(function (d, j) {
                ...
                domain = range(colors.length).map(function (d, i) {
                    return start + i * step;
                });
            }
        ...
        function highlightPoint(pos, bucket) {
            ...
            var highlightColor = d3Color(color).darker(1);
        ...
        function drawColorLegend() {
            ...
            var legend = d3Select("#heatmap-color-legend");
            var legendWidth = Math.floor($(d3Select("#heatmap-color-legend").node()).outerWidth());
            var legendHeight = d3Select("#heatmap-color-legend").attr("height");
        ...
        function addLegend() {
            ...
            var legendScale = scaleLinear().domain([min, max]).range([0, legendWidth]);

            var legendAxis = axisBottom(legendScale).ticks(20).tickFormat(formatter).tickSize(legendHeight);
        ...
        function drawLegend(legend, legendWidth, legendHeight) {
            ...
            var positionRange = range(0, legendWidth, rangeStep);

            var valueScale = scaleLinear().domain([0, legendWidth]).range([min, max]);
    ...
    return {
        setters: [function (_d2) {
            axisBottom = _d2.axisBottom;
            d3AxisLeft = _d2.axisLeft;
            d3Color = _d2.color;
            scaleLinear = _d2.scaleLinear;
            scaleSequential = _d2.scaleSequential;
            scaleTime = _d2.scaleTime;
            d3Select = _d2.select;
            timeDay = _d2.timeDay;
            timeFormat = _d2.timeFormat;
            timeHour = _d2.timeHour;
            timeMonday = _d2.timeMonday;
            timeMonth = _d2.timeMonth;
            timeSaturday = _d2.timeSaturday;
        }, function (_lodash) {
            _ = _lodash.default;
            range = _lodash.range;
        }, function (_moment) {
